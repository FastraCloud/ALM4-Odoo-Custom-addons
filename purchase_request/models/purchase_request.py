...
class PurchaseRequest(models.Model):
	...
	return self.env['res.users'].browse('purchase_request.group_purchase_request_manager')
    @api.model
    ...
    def _compute_purchase_orders(self):
        for rec in self:
            ...
            purchase_orders = self.env['purchase.order'].search([('purchase_request_ids','=',rec.id)])
            lines = []
            ...
            rec.purchase_order_count = len(lines)
    ...
    assigned_to = fields.Many2one('res.users', 'Approver',
                                  track_visibility='onchange', domain=[('groups_id','in',[72])])
    ...
    line_ids = fields.One2many('purchase.request.line', 'request_id',
                               ...
                               copy=True,
                               track_visibility='onchange')
    purchase_order_ids = fields.Many2many('purchase.order',compute='_compute_purchase_orders',string="Purchase Orders")
    ...
    def action_view_purchase_orders(self):
        ...
        result.pop('id', None)
        result['context'] = {}
        po_ids = sum([pr.purchase_order_ids.ids for pr in self], [])
	...
	# def _check_for_different_tag(self):
        """for r in self:
            lines = r.line_ids
            analytic_tag = lines[0].analytic_tag_ids
	    analytic_account = lines[0].analytic_account_id
            for line in lines:
                if analytic_tag != line.analytic_tag_ids or analytic_account !=  line.analytic_account_id:
                   raise exceptions.ValidationError("You cannot use different analytic Tag or  analytic account. please check the list and correct the errors")"""
